<cfcomponent displayname="PDFUtility" output="false"
    ...
    <cffunction name="addSignatureLine" access="public" returntype="string" output="false"
        ...
        <cfif NOT fileExists(arguments.SignaturePDFPath)>
            <cfthrow type="PDFUtility.InvalidArgument"
                message="Signature template PDF does not exist."
                detail="SignaturePDFPath: #arguments.SignaturePDFPath#" />
        </cfif>
    ...
    <cffunction name="addSignatureText" access="public" returntype="string" output="false"
        hint="Writes SignatureText and today's date below the stamped signature image on the last page.">
        ...
        <cftry>
            ...
            <cfset var lastPage  = doc.getPage(javaCast("int", doc.getNumberOfPages() - 1)) />
            <cfset var pageWidth = lastPage.getMediaBox().getWidth() />

            <!---
            Replicate Phase 1 pixel-scan from addSignatureLine to compute
            the exact stamp position that was used on the source PDF.
            --->
            <cfset var SCAN_DPI  = 36  />
            <cfset var THRESHOLD = 240 />
            <cfset var PAD_PX    = 4   />
            ...
            <!--- Natural stamp size in PDF points (mirrors addSignatureLine Phase 3) --->
            <cfset var cropW  = bMaxX - bMinX + 1 />
            <cfset var cropH  = bMaxY - bMinY + 1 />
            <cfset var stampW = cropW * 72.0 / VARIABLES.RENDER_DPI />
            <cfset var stampH = cropH * 72.0 / VARIABLES.RENDER_DPI />
            <cfset var stampX = pageWidth - stampW - VARIABLES.SIG_MARGIN />
            <cfset var stampY = VARIABLES.SIG_MARGIN />
            ...
            <!---
            Right-align name and date to the same right edge as the signature image.
            Stack them vertically (name closest to image, date below) so they never
            overlap.  X for each is computed from its actual rendered text width —
            no fixed offsets.
            --->
            <cfset var font      = PDType1Font.init(FontName.HELVETICA_BOLD) />
            <cfset var rightEdge = pageWidth - VARIABLES.SIG_MARGIN />
            <cfset var lineH     = VARIABLES.FONT_SIZE + 3 />

            <cfset var nameWidth = font.getStringWidth(arguments.SignatureText) / 1000.0 * VARIABLES.FONT_SIZE />
            <cfset var dateWidth = font.getStringWidth(currentDate)              / 1000.0 * VARIABLES.FONT_SIZE />

            <cfset var nameY = max(4, stampY - lineH) />
            <cfset var dateY = max(4, nameY  - lineH) />
            <cfset var nameX = rightEdge - nameWidth />
            <cfset var dateX = rightEdge - dateWidth />

            <cfset var cs = PDPageCS.init(
                doc, lastPage, AppendMode.APPEND,
                javaCast("boolean", true), javaCast("boolean", true)
            ) />
            ...
            <cftry>
                ...
                <cfset cs.newLineAtOffset(javaCast("float", nameX), javaCast("float", nameY)) />
                <cfset cs.showText(arguments.SignatureText) />
                <cfset cs.endText() />

                <cfset cs.beginText() />
                <cfset cs.setFont(font, javaCast("float", VARIABLES.FONT_SIZE)) />
                <cfset cs.newLineAtOffset(javaCast("float", dateX), javaCast("float", dateY)) />
            ...
            <cffinally>
                <cfif isObject(doc)>
                    <cftry><cfset doc.close() /><cfcatch type="any" /></cftry>
                </cfif>
                <cfif isObject(sigDoc)>
                    <cftry><cfset sigDoc.close() /><cfcatch type="any" /></cftry>
                </cfif>
